...
original_name="edgar_news"
original_urlname="edgar-news"
original_description="Awesome edgar_news created by butzhang" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/edgar_news "${name}"
...
then
    ...
    echo "then run 'edgar_news' or 'python -m edgar_news'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'edgar_news' or 'python -m edgar_news' or 'poetry run edgar_news'"
fi

...
- 🎯 Entry points to execute your program using `python -m <edgar_news>` or `$ edgar_news` with basic CLI argument parsing.
...
---
# edgar_news Flask Application

Awesome edgar_news created by butzhang
...
```bash
git clone https://github.com/butzhang/edgar-news edgar_news
cd edgar_news
make install
```
...
```bash
edgar_news create-db   # run once
edgar_news populate-db  # run once (optional)
edgar_news add-user -u admin -p 1234  # ads a user
edgar_news run
```
